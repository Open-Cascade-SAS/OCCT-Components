...
#ifndef _TKDERVM_HeaderFile
#define _TKDERVM_HeaderFile

#include <Standard_Macro.hxx>
#include <TCollection_AsciiString.hxx>

//! Interface to activate license for TKDERVM toolkit
...
{
public:
  //! Activates the application license for a current product by a key string or license path or
  //! server. The call should be made prior to usage of the corresponding product. Only the one call
  ...
  //! @param[in] theNetworkAdapterNum used only on Windows machines to get host ID by specified
  ...
  //! @param[out] theLicensesInfo info about avialable licenses.
  ...
  //! In case of license file or server acrivates toolkits with it as a global license server or
  //! file The keys or server or file should be loaded before any calls to corresponding products.
  //! Consequtive calls are possible, allowing to load several files.
  ...
  //! @param[in] thePrefix license prefix, which is prepended to each feature name during license
  //! checking out
  Standard_EXPORT static void SetGlobalLicensePrefix(const TCollection_AsciiString& thePrefix);
};

#endif // _TKDERVM_HeaderFile
