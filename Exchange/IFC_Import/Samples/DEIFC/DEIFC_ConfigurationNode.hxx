...
#ifndef _DEIFC_ConfigurationNode_HeaderFile
#define _DEIFC_ConfigurationNode_HeaderFile

#include <DE_ConfigurationNode.hxx>

//! The purpose of this class is to configure the transfer process for IFC format
//! Stores the necessary settings for DEIFC_Provider.
//! Configures and creates special provider to transfer IFC files.
//!
//! Nodes grouped by Vendor name and Format type.
//! The Vendor name is "OCC"
//! The Format type is "IFC"
//! The supported CAD extension is ".ifc"
//! The import process is supported.
//! The export process isn't supported.
class DEIFC_ConfigurationNode : public DE_ConfigurationNode
{
  DEFINE_STANDARD_RTTIEXT(DEIFC_ConfigurationNode, DE_ConfigurationNode)
public:
  //! Initializes all fields by default
  Standard_EXPORT DEIFC_ConfigurationNode();

  //! Copies values of all fields
  //! @param[in] theNode object for copy
  Standard_EXPORT DEIFC_ConfigurationNode(const Handle(DEIFC_ConfigurationNode)& theNode);
...
public:
  struct IFCCAFControl_InternalSection
  {
    bool ReadDiscardProjTrsf =
      false; //<! Set flag to ignore project (root) transformation; FALSE by default
    bool ReadExpandProdRepr = true; //<! Sets the option for enabling/disabling the expansion of the
                                    //Product Representation items
    bool ReadUseTypeAsName =
      false; //<! Using type of the IFC entity as a prefix for its name in assembly structure
    double ReadAngleDeflection = 45.0; //<! Set default value for angle deflection
  };

public:
  IFCCAFControl_InternalSection InternalParameters; //<! Customize transfer process parameters

private:
  Standard_Boolean myIsValidReadLicense; //!< Valid flag of the license to read
};

#endif // _DEIFC_ConfigurationNode_HeaderFile
